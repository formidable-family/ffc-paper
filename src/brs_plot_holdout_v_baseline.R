...
# leaderboard scores
scores <- read_csv(file.path("data", "ffc_models_leaderboard_scores_brs.csv"))
...
scores <- 
  ...
  mutate(
    vars_and_scores = case_when(
      model == "bayesian rule set" & scores == "none" ~ "Bayesian rule set", 
      model == "bayesian rule set" & scores == "expert" ~ "brs + expert scores",
...
write_csv(scores, "data/brs_holdout_scores_wide.csv")
write_csv(scores_long, "data/brs_holdout_scores_long.csv")
...
ggsave(filename = file.path("output", "brs_holdout_v1.png"), 
       plot = holdout_v1, 
       height = 5, width = 8)
